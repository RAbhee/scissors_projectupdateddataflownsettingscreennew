...
import 'package:scissors_project/Screens/dashboard/add_profile.dart';
...
class _ListScreenState extends State<ListScreen> {
  ...
  void initState() {
    super.initState();
    servicesStream = FirebaseFirestore.instance.collection('services').snapshots();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: StreamBuilder(
            ...
            builder: (context, snapshot) {
              ...
              List<Services> servicesList = snapshot.data!.docs.map((
    ...
    totalAmount +=
    double.tryParse(service.price.replaceAll(RegExp(r'[^\d.]'), '')) ?? 0.0;
    _showPriceUpdateForm(service.id);

    addService(service.name, service.description, service.price, service.images);
    }
    });
